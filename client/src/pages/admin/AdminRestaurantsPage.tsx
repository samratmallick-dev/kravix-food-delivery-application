...
import { getAllRestaurants, getRestaurantById, verifyRestaurant, deleteRestaurant, approveLocationUpdate, rejectLocationUpdate } from "@/services/api/admin.services";
...
const AdminRestaurants = () => {
      ...
      return (
            <div className="p-6 space-y-5">
                  ...
                  <AdminTable columns={columns} data={restaurants} loading={loading} page={page} pages={pages} total={total} onPageChange={setPage} keyExtractor={(r: any) => r._id} />
                        ...
                        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setRejectingLocation(null)}>
                              <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
                                    ...
                                    <textarea
                                          ...
                                          className="w-full text-sm border border-border rounded-xl p-3 focus:outline-none focus:border-primary min-h-20"
                                    />
                                    <div className="flex gap-3">
                                          <button
                                                onClick={() => { setRejectingLocation(null); setRejectReason(""); }}
                                                className="flex-1 py-2.5 rounded-xl border border-border text-sm font-medium text-gray-600 hover:bg-gray-50 transition cursor-pointer"
                                          >
                                                Cancel
                                          </button>
                                          <button
                                                onClick={() => handleRejectLocation(rejectingLocation, rejectReason)}
                                                disabled={!rejectReason.trim()}
                                                className="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold transition disabled:opacity-60 cursor-pointer"
                                          >
                                                Decline
                                          </button>
                                    </div>
