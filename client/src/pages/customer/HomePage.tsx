...
import Hero from "@/features/customer/components/Hero";
import { useAppData } from "@/context/AppContext";
import type { IRestaurant } from "@/types";
import { useEffect, useRef, useState } from "react";
import toast from "react-hot-toast";
import { getNearestRestaurant } from "@/services/api/restaurant.services";
import FeatureBanmner from "@/features/customer/components/FeatureBanner";
...
import { getAvailableCategories } from "@/services/api/menu.services";
import type { ICategory } from "@/services/api/menu.services";
...
const Home = () => {
      ...
      const { location, locationLoading, detectUserLocation, city } = useAppData();
      ...
      const fetchRestaurantRef = useRef(fetchRestaurant);
      useEffect(() => {
            fetchRestaurantRef.current = fetchRestaurant;
      }, [fetchRestaurant]);

      const joinedRooms = useRef<Set<string>>(new Set());

      useEffect(() => {
            if (!socket || !retaurants.length) return;
            retaurants.forEach((r) => {
      ...
      const currentOrigin = typeof window !== "undefined" ? window.location.origin : "https://kravix-nu.vercel.app";
            ...
            "@graph": [
                  {
                        ...
                        "name": "Kravix",
                        "url": `${currentOrigin}/`,
                        "logo": `${currentOrigin}/android-chrome-512x512.png`,
                        "sameAs": [
                              "https://github.com/samratmallick-dev/kravix-food-delivery-application"
                        ]
                  },
                  {
                        "@type": "WebSite",
                        ...
                        "potentialAction": {
                              ...
                              "target": {
                                    "@type": "EntryPoint",
                                    "urlTemplate": `${currentOrigin}/search?search={search_term_string}`
                              },
                  ...
                  },
                  {
                        ...
                        "@id": `${currentOrigin}/#fooddeliveryservice`,
                        ...
                        ...(retaurants.length > 0 ? {
                              "hasOfferCatalog": {
                                    "@type": "OfferCatalog",
                                    "name": `Restaurants open near ${city || "you"}`,
                                    "itemListElement": retaurants.map((r) => ({
                                          "@type": "Offer",
                                          "itemOffered": {
                                                "@type": "Restaurant",
                                                "name": r.name,
                                                "image": r.image,
                                                "description": r.description || `Order food from ${r.name}`,
                                                "url": `${currentOrigin}/restaurant/${r.slug || r._id}`,
                                                "telephone": r.phone ? `${r.phone}` : undefined
                                          }
                                    }))
                              }
                        } : {})
                                                ...
                                                categories.slice(0, 5).map((cat) => {
                                                      ...
                                                      return (
                                                            <button
                                                                  ...
                                                                  className={`px-3 py-1.5 rounded-full border transition-all duration-200 shadow-2xs hover:scale-[1.02] active:scale-[0.98] cursor-pointer font-bold text-[11px] ${isActive
                              ...
                              <div className="max-w-2xl mx-auto space-y-4">
                                    {[
                                          ...
                                          return (
                                                <div
                                                      key={index}
                                                      className={`bg-white rounded-2xl border transition-all duration-300 overflow-hidden shadow-3xs ${isOpen ? "border-primary/20 shadow-xs ring-2 ring-primary/5" : "border-gray-100 hover:border-gray-250"
                                                            }`}
                                                >
                                                      <button
                                                            type="button"
                                                      ...
                                                      >
                                                            ...
                                                            <ChevronDown
                                                                  size={16}
                                                                  className={`text-primary transition-transform duration-300 shrink-0 ml-4 ${isOpen ? "rotate-180" : ""
                                                                        }`}
                                                            />
                                                      </button>
                                                      <div
