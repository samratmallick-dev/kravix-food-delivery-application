...
import { useDeliveryTimer } from "@/features/rider/hooks/useDeliveryTimer";
...
const CurrentOrderCard = memo(({ order, onStatusUpdate, onGenerateOtp }: CurrentOrderCardProps) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-(--shadow-md) overflow-hidden rider-page-enter">
      {/* Header */}
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex items-center justify-between bg-gray-50 rounded-xl px-3 py-2.5">
          ...
          <a href={`tel:${order.deliveryAddress?.mobile || ""}`}
            className="text-xs font-semibold px-3 py-1.5 rounded-lg bg-primary text-white hover:bg-red-700 transition min-w-11 min-h-11 flex items-center justify-center"
            ...
            <div className="grid grid-cols-2 gap-2">
              {COD_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm font-medium transition min-h-11 ${selectedCodMode === opt.value ? "bg-orange-500 text-white border-orange-500" : "bg-white text-gray-700 border-gray-200 hover:border-orange-300"}`}
                >
