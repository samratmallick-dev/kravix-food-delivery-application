...
const RegistrationForm = ({ fetchProfile }: { fetchProfile: () => Promise<void> }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      await addRiderProfile({ phoneNumber, aadhaarNumber, drivingLicesce, ...(panNumber && { panNumber: panNumber.toUpperCase() }), image, latitude: location.latitude, longitude: location.longitude });
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-[var(--shadow-lg)] overflow-hidden">
        <div className="px-8 py-6" style={{ background: "var(--gradient-rider)" }}>
        ...
        <div className="px-8 py-6 space-y-5">
          <label className="block cursor-pointer group">
            <div className={`relative w-full h-44 rounded-xl border-2 border-dashed transition-colors overflow-hidden flex items-center justify-center ${preview ? "border-primary" : "border-gray-300 hover:border-primary"}`}>
              {preview ? (
          ...
          {[
            { label: "Phone Number",    value: phoneNumber,   onChange: setPhoneNumber,   icon: <Phone size={16} />,      placeholder: "+91-9999999999" },
            { label: "Aadhaar Number",  value: aadhaarNumber, onChange: setAadhaarNumber, icon: <CreditCard size={16} />, placeholder: "XXXX-XXXX-XXXX" },
            { label: "Driving License", value: drivingLicesce, onChange: setDrivingLicesce, icon: <FileText size={16} />,  placeholder: "WB-0420110012345" },
            { label: "PAN Number",      value: panNumber,     onChange: setPanNumber,     icon: <CreditCard size={16} />, placeholder: "ABCDE1234F" },
          ].map(({ label, value, onChange, icon, placeholder }) => (
            <div key={label} className="space-y-1">
              <label className="text-sm font-medium text-gray-700">{label} <span className="text-primary">*</span></label>
...
const RiderTopBar = memo(({ mobileMenuOpen, onMobileMenuToggle }: { mobileMenuOpen: boolean; onMobileMenuToggle: () => void }) => (
  <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-md border-b border-gray-100 px-4 py-3 flex items-center justify-between lg:hidden">
    <div className="flex items-center gap-2">
      <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "var(--gradient-rider)" }}>
...
const MobileDrawer = memo(({ open, onClose }: { open: boolean; onClose: () => void }) => (
  <>
    ...
    <div className={`fixed top-0 right-0 h-full w-64 bg-white z-50 shadow-[var(--shadow-lg)] transition-transform duration-300 lg:hidden ${open ? "translate-x-0" : "translate-x-full"}`}>
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <span className="font-bold text-gray-800">Menu</span>
        <button onClick={onClose} aria-label="Close menu" className="p-1 rounded-lg text-gray-500 hover:bg-gray-100">
          <X size={18} />
        </button>
      </div>
      <nav className="py-4 px-3 space-y-1" aria-label="Mobile rider navigation">
        {NAV_ITEMS.map(({ to, label, icon: Icon }) => (
          <NavLink
            key={to}
            to={to}
  ...
  >
    {BOTTOM_NAV.map(({ to, label, icon: Icon }) => (
      <NavLink
        ...
        to={to}
        className={({ isActive }) =>
          `flex-1 flex flex-col items-center justify-center py-2 gap-0.5 text-[10px] font-semibold transition min-h-[56px] ${isActive ? "text-primary" : "text-gray-400 hover:text-gray-600"}`
        }
        aria-label={label}
      >
        {({ isActive }) => (
...
const RiderLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <RiderSidebar collapsed={sidebarCollapsed} onToggle={() => setSidebarCollapsed((v) => !v)} />
      <RiderTopBar mobileMenuOpen={mobileMenuOpen} onMobileMenuToggle={() => setMobileMenuOpen((v) => !v)} />
