...
export const startEmailConsumer = async (): Promise<void> => {
    ...
    async (msg) => {
      ...
      try {
        if (payload.type === "VERIFICATION") {
          ...
          console.warn(
            "[Email Consumer] Unknown job type:",
            (payload as EmailJobPayload).type,
          );
          channel.nack(msg, false, false);
          return;
        }
        channel.ack(msg);
        console.log(
          `[Email Consumer] ✅ Sent ${payload.type} email to ${payload.to}`,
        );
      } catch (err) {
        console.error(
  ...
  console.log(
    `✅ [Email Service] Consuming from queue: ${process.env.EMAIL_QUEUE}`,
  );
};