...
export const publishAuthEvent = (
      ...
      axios
            ...
            .catch((httpErr: unknown) => {
                  ...
                  console.error(
                        `[Auth] ⚠️ Realtime HTTP call failed for "${socketEvent}", falling back to RabbitMQ: ${msg}`,
                  );
                  ...
                  try {
                        ...
                        if (sent) {
                              console.log(
                                    `[Auth] 📤 Fallback: published "${type}" to AUTH_EVENT_QUEUE`,
                              );
                        } else {
                              console.warn(
                                    `[Auth] ⚠️ RabbitMQ buffer full — fallback event "${type}" was not sent`,
                              );
                        }
                  } catch (mqErr) {
                        console.error(
                              `[Auth] ❌ Both Realtime HTTP and RabbitMQ failed for "${type}":`,
                              mqErr,
                        );
                  }
            });
