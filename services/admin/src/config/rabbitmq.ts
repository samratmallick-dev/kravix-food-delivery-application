...
export const connectRabbitMQ = async () => {
      try {
            ...
            console.error(
                  "Error while connecting to RabbitMQ in Admin Service:",
                  error,
            );
...
export const getRabbitMQChannel = (): amqp.Channel => {
      if (!channel)
            throw new Error("RabbitMQ channel is not initialized in Admin Service");
      return channel;
};

export const publishAdminEvent = (
      type: string,
      ...
      if (!sent) {
            console.error(
                  `RabbitMQ buffer full — failed to publish admin event "${type}"`,
            );
      } else {
