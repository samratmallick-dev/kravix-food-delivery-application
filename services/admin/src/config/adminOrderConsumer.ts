...
export const startAdminOrderConsumer = async () => {
      ...
      channel.consume(
            ...
            async (msg) => {
                  ...
                  const primaryId = (parsed.data?.orderId ??
                  ...
                  if (isDuplicate(idempotencyKey)) {
                        console.log(
                              "Admin consumer: duplicate event skipped —",
                  ...
                  try {
                        switch (parsed.type) {
                              ...
                              case "RESTAURANT_VERIFIED":
                                    console.log(
                                          "🏪 Admin event — restaurant verified:",
                                          sanitize(parsed.data.restaurantId),
                                    );
                              ...
                              case "RIDER_VERIFIED":
                                    console.log(
                                          "🏍️ Admin event — rider verified:",
                                          sanitize(parsed.data.riderId),
                                    );
                              ...
                              case "RESTAURANT_DELETED":
                                    console.log(
                                          "🗑️ Admin event — restaurant deleted:",
                                          sanitize(parsed.data.restaurantId),
                                    );
                              ...
                              case "USER_BLOCK_STATUS_CHANGED":
                                    console.log(
                                          "🚫 Admin event — user block status changed:",
                                          sanitize(parsed.data.userId),
                                    );
                                    ...
                                    axios
                                          ...
                                          .catch((err) =>
                                                console.error("User block socket emit failed:", err.message),
                                          );
                                    if (parsed.data.role === "seller" && parsed.data.restaurantId) {
                                          axios
                                                ...
                                                .catch((err) =>
                                                      console.error(
                                                            "Restaurant block socket emit failed:",
                                                            err.message,
                                                      ),
                                                );
                                    }
                              ...
                              default:
                                    console.log(
                                          "Admin consumer: unknown event type — skipping:",
                                          sanitize(parsed.type),
                                    );
                        }
