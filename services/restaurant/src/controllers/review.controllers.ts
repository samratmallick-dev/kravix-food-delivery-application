...
export const createReview = TryCatch(
      async (req: AuthenticatedRequest, res: Response) => {
            ...
            if (!["restaurant", "menu_item", "rider"].includes(type)) {
                  return res
                        .status(400)
                        .json({ message: "Invalid review type", success: false, error: true });
            }
            ...
            if (!order) {
                  return res
                        .status(404)
                        .json({ message: "Order not found", success: false, error: true });
            }
            ...
            if (existingReview) {
                  return res
                        .status(400)
                        .json({
                              message: `You have already reviewed the ${type} for this order`,
                              success: false,
                              error: true,
                        });
            }
...
export const getRestaurantReviews = TryCatch(
      async (req: AuthenticatedRequest, res: Response) => {
            ...
            ratingsSummary.averageRating =
                  Math.round(ratingsSummary.averageRating * 10) / 10;
...
export const getRiderReviews = TryCatch(
      async (req: AuthenticatedRequest, res: Response) => {
            const { id } = req.params;
...
export const reportReview = TryCatch(
      async (req: AuthenticatedRequest, res: Response) => {
            const user = req.user;
            if (!user) {
                  return res
            ...
            if (!review) {
                  return res
                        .status(404)
                        .json({ message: "Review not found", success: false, error: true });
            }
...
export const getAdminReviews = TryCatch(
      async (req: AuthenticatedRequest, res: Response) => {
            const user = req.user;
...
export const moderateReview = TryCatch(
      async (req: AuthenticatedRequest, res: Response) => {
            const user = req.user;
            if (!user || user.role !== "admin") {
                  return res
