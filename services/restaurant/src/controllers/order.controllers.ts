            ...
            if (couponCode) {
                  ...
                  if (coupon && new Date(coupon.expiryDate) >= new Date()) {
                        if (
                              ...
                              coupon.restaurantId === restaurantId.toString()
                        ) {
                              if (subTotal >= coupon.minOrderAmount) {
                                    ...
                                    if (coupon.perUserLimit > 0) {
                                          const count = await CouponUsage.countDocuments({
                                                couponId: coupon._id,
                                                userId: user._id,
                                          });
      ...
      async (req: AuthenticatedRequest, res: Response) => {
            ...
            if (restaurant?.ownerId !== user._id.toString()) {
                  return res.status(403).json({
                        success: false,
                        message:
                              "Access denied. You don't have permission to update this order.",
                        error: true,
                  });
            }
            ...
            await Promise.all([
                  ...
                  axios.post(
                        emitUrl,
                        { event: "order:update", room: "Admin", payload: statusPayload },
                        { headers: emitHeaders },
                  ),
            ]);
...
export const assignRiderToOrder = TryCatch(async (req, res) => {
      ...
      const { orderId, riderId, riderUserId, riderName, riderPhoneNumber } =
            req.body;
      ...
      await Promise.all([
            ...
            axios.post(
                  emitUrl,
                  { event: "order:update", room: "Admin", payload: assignPayload },
                  emitOpts,
            ),
      ]);
...
export const updateOrderStatusByRider = TryCatch(async (req, res) => {
      ...
      if (nextStatus) {
            ...
            await Promise.all([
                  ...
                  axios.post(
                        riderEmitUrl,
                        { event: "order:update", room: "Admin", payload: socketPayload },
                        { headers: riderEmitHeaders },
                  ),
            ]);
...
export const setOrderOtp = TryCatch(async (req, res) => {
      if (req.headers["x-internal-key"] !== process.env.INTERNAL_SERVICE_KEY) {
            return res
                  .status(403)
                  .json({ success: false, message: "Forbidden", error: true });
      }
...
export const getOrderByIdInternal = TryCatch(async (req, res) => {
      ...
      const order = await Order.findById(req.params.orderId).select(
            "status riderId userId",
      );
      if (!order) {
            return res
                  .status(404)
                  .json({ success: false, message: "Order not found", error: true });
      }
...
export const getRestaurantSalesStats = TryCatch(
      async (req: AuthenticatedRequest, res: Response) => {
            ...
            if (!restaurant || restaurant.ownerId !== user._id.toString()) {
                  return res
                        .status(403)
                        .json({ success: false, message: "Access denied", error: true });
            }
...
export const getDeliveredOrdersByRider = TryCatch(async (req, res) => {
      ...
      if (!riderId) {
            return res
                  .status(400)
                  .json({ success: false, message: "Rider Id is required", error: true });
      }
...
export const reorderItems = TryCatch(
      async (req: AuthenticatedRequest, res: Response) => {
            const user = req.user;
            if (!user) {
                  return res
            ...
            if (!restaurant) {
                  return res
                        .status(404)
                        .json({ success: false, message: "Restaurant not found", error: true });
            }
            ...
            const availableItems = order.items.filter((i) =>
                  availableItemMap.has(i.itemId),
            );
...
export const cancelMyOrder = TryCatch(
      async (req: AuthenticatedRequest, res: Response) => {
            const user = req.user;
            if (!user) {
                  return res.status(401).json({
            ...
            if (order.userId !== user._id.toString()) {
                  return res.status(403).json({
                        success: false,
                        message:
                              "Access denied. You don't have permission to cancel this order.",
                        error: true,
                  });
            }
