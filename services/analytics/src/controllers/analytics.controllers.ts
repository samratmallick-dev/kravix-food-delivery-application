...
export const getDashboardAnalytics = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      const user = req.user;
      if (!user) {
            res.status(401).json({ message: "User not authenticated", success: false, error: true });
            return;
      }

      if (user.role !== "admin" && user.role !== "seller") {
            res.status(403).json({ message: "Access denied. Admins or sellers only.", success: false, error: true });
            return;
      }
      ...
      if (user.role === "seller") {
            if (!user.restaurantId) {
                  res.status(400).json({ message: "Seller has no associated restaurant", success: false, error: true });
                  return;
            }
      ...
      try {
            ...
            const [
                  ...
                  !restaurantId ? computeTopRestaurants(start, end, 5) : Promise.resolve([]),
            ...
            res.status(500).json({ message: "Failed to compute dashboard analytics", success: false, error: true });
      }
};

export const exportRevenueTrendsCSV = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
      const user = req.user;
      if (!user) {
            res.status(401).json({ message: "User not authenticated", success: false, error: true });
            return;
      ...
      try {
            const trends = await computeRevenueTrends(start, end, interval as any, restaurantId as string);
            ...
            res.setHeader("Content-Disposition", `attachment; filename="revenue_trends_${interval}.csv"`);
            res.status(200).send(csv);
      } catch (error) {
            console.error("Failed to export analytics CSV:", error);
            res.status(500).json({ message: "Failed to export analytics CSV", success: false, error: true });
      }
};
